...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ${train_set} dev test; do
        ...
        steps/make_fbank.sh --cmd "$train_cmd" --nj $nj \
            --write_utt2num_frames true --fbank_config $fbank_conf --compress $compress $feat_dir/$x >> nohup.make_fbank_${x} 2>&1
    done
    if $cmvn; then
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for x in dev test ${train_set}; do
        ...
        tools/format_data.sh --nj ${nj} --feat $feat_dir/$x/feats.scp \
            --bpecode data/dict/bpe5000.model --trans_type cn_char_en_bpe \
            fbank/${x} data/dict/char6611_bpe5000.txt > fbank/${x}/format.data.char11_bpe5000.6611
    done
fi
