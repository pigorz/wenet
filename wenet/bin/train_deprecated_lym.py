...
the future, please move to the new IO !!!
    ...
    parser.add_argument("--encoder_finetune",default=0,type=int)
    parser.add_argument("--en_ckpt",default='pretrain-ckpt/en.pt',type=str)
    parser.add_argument("--ch_ckpt",default='pretrain-ckpt/cn.pt',type=str)
    ...
    configs['mix'] = args.mix
    configs['encoder_finetune'] = args.encoder_finetune == 1
    configs['en_ckpt'] = args.en_ckpt
    configs['ch_ckpt'] = args.ch_ckpt
    configs['fusion'] = args.fusion
    configs['gate'] = args.gate
    ...
    num_params_ctc = sum(p.numel() for p in model.ctc.parameters())
    print('the number of ctc params: {}'.format(num_params_ctc))
    ...
    # Start training loop
    executor.step = step
    executor._iteration(configs['dataset_conf']['batch_size'],len(train_dataset))
